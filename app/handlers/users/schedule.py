...
async def schedule_handler(message: Message, group_id, state: FSMContext):
    ...
    text, markup = await _get_schedule_data(group_id)
...
async def schedule_callback(call: CallbackQuery, group_id):
    ...
    text, markup = await _get_schedule_data(group_id, call.data[9:])
    try:
...
async def _get_schedule_data(group_id: int, shift: str = 'this', *args, **kwargs) -> (str, any):
