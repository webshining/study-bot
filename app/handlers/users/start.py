...
async def start_handler(message: Message, user):
    ...
    commands = get_admins_commands(message.from_user.language_code) if user.status in ('admin', 'super_admin') else get_default_commands(message.from_user.language_code)
    ...
    await message.answer(text, disable_web_page_preview=True, reply_markup=None)
