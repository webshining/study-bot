...
async def login(request: Request, redirect: str = ""):
    ...
    return templates.TemplateResponse('login.html', {"bot_username": bot_username, "redirect": request.url_for('login_redirect').include_query_params(redirect=redirect), "request": request})
...
async def login_redirect(id: int, redirect: str, request: Request):
    if is_telegram(dict(request.query_params)):
        ...
        data = {"user": model_to_dict(user), "accessToken": access_token}
        if redirect:
            response = RedirectResponse(f'{redirect}#{json.dumps(data)}')
        else:
            response = JSONResponse(data)
        response.set_cookie(key='refreshToken', value=refresh_token, max_age=REFRESH_TOKEN_EXPIRE_MINUTES*60, httponly=True, secure=True, samesite='none')
        return response
...
async def refresh(request: Request):
    ...
    response.set_cookie(key='refreshToken', value=refresh_token, secure=True, httponly=True, max_age=REFRESH_TOKEN_EXPIRE_MINUTES*60, samesite='none')
    return response
